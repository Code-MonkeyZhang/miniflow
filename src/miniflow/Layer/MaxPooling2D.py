...
class MaxPooling2D(Layer):
    def __init__(self, pool_size, input_shape, stride=None, padding="valid", layer_name='MaxPooling2D'):
        super().__init__(layer_name)

        self.input_shape = input_shape
        self.pool_size = pool_size
        self.padding = padding
        self.stride = stride if stride is not None else pool_size  # if stride is not assigned, set stride = pool_size by default
    ...
    def compute_layer(self, a_in: np.ndarray) -> np.ndarray:
        ...
        output = np.zeros((num_example, out_h, out_w, in_ch))
        ...
        for h in range(out_h):
            for w in range(out_w):
                ...
                output[:, h, w, :] = np.max(a_prev_slice, axis=(1, 2))
